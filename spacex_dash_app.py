...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        class_mean = spacex_df.loc[spacex_df["Launch Site"] == entered_site, ['class']].mean()
        filtered_df = pd.DataFrame()
        filtered_df['class'] = [1, 0]
        filtered_df['percent'] = [float(class_mean), float(1-class_mean)]
...
def get_scatter_chart(entered_site, payload_mass):
    filtered_df = spacex_df.loc[((np.float64(spacex_df["Payload Mass (kg)"]) >= np.float64(payload_mass[0])) & (np.float64(spacex_df["Payload Mass (kg)"]) <= np.float64(payload_mass[1]))), ['Launch Site', 'class', 'Payload Mass (kg)', 'Booster Version Category']]
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df, y='class', x='Payload Mass (kg)', color="Booster Version Category",
        title='Correlation between Payload and Success for all sites')
        return fig
    else:
        filtered_df = filtered_df.loc[filtered_df["Launch Site"] == entered_site, ['class', 'Payload Mass (kg)', 'Booster Version Category']]
        fig = px.scatter(filtered_df, y='class', x='Payload Mass (kg)', color="Booster Version Category",
